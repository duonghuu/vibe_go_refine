  ...
  return <Grid2 container spacing={3}>
    <Grid2 size={{ xs: 12, md: 6 }}>
      <Card variant="outlined"><CardContent><Typography variant="h6" mb={2}>Thông tin chung</Typography><Stack spacing={2}>
        <TextField label="Mã loại" InputLabelProps={{ shrink: true }} disabled={isEdit} fullWidth {...register("code", { required: "Mã loại là bắt buộc", pattern: { value: /^[A-Z][A-Z0-9_]{0,49}$/, message: "Dùng chữ in hoa, số và dấu gạch dưới" } })} error={Boolean(errors.code)} helperText={errors.code?.message} onChange={(event) => { if (!isEdit) setValue("code", event.target.value.toUpperCase(), { shouldDirty: true, shouldValidate: true }); }} />
        <TextField label="Tên loại" InputLabelProps={{ shrink: true }} fullWidth {...register("name", { required: "Tên loại là bắt buộc", maxLength: { value: 100, message: "Tối đa 100 ký tự" } })} error={Boolean(errors.name)} helperText={errors.name?.message} />
        <TextField label="Thứ tự" InputLabelProps={{ shrink: true }} type="number" fullWidth {...register("sortOrder", { valueAsNumber: true, min: { value: 0, message: "Phải lớn hơn hoặc bằng 0" } })} error={Boolean(errors.sortOrder)} helperText={errors.sortOrder?.message} />
      </Stack></CardContent></Card>
    </Grid2>
    <Grid2 size={{ xs: 12, md: 6 }}>
      <Card variant="outlined"><CardContent><Typography variant="h6" mb={2}>Trạng thái và giới hạn</Typography><Stack spacing={2}>
        <TextField select label="Trạng thái" InputLabelProps={{ shrink: true }} fullWidth {...register("status", { required: true })}><MenuItem value="ACTIVE">Đang hoạt động</MenuItem><MenuItem value="INACTIVE">Tạm ẩn</MenuItem></TextField>
        ...
        {limitEnabled && <TextField label="Số lượng tối đa" InputLabelProps={{ shrink: true }} type="number" fullWidth {...register("maxItems", { valueAsNumber: true, required: "Nhập giới hạn", min: { value: 1, message: "Từ 1 đến 1000" }, max: { value: 1000, message: "Từ 1 đến 1000" } })} error={Boolean(errors.maxItems)} helperText={errors.maxItems?.message} />}
      </Stack></CardContent></Card>
    </Grid2>
    <Grid2 size={12}>
