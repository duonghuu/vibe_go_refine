...
export const ImageContentList = () => {
  ...
  const queryType = params.get("typeCode") ?? "";
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("");
  const [typeCode, setTypeCode] = useState(queryType);
  ...
  useEffect(() => {
    ...
    if (typeCode)
      filters.push({ field: "typeCode", operator: "eq", value: typeCode });
    setFilters(filters);
  }, [debouncedSearch, setFilters, status, typeCode]);
  const columns = useMemo<GridColDef<IImageContent>[]>(
    () => [
      ...
      },
      {
        ...
        renderCell: ({ row }) => (
          <Typography sx={{
              ...
              alignItems: "center",
              alignContent: "center",
            }} fontWeight={600} noWrap title={row.name ?? undefined}>
            {row.name || `${row.typeName} #${row.id}`}
          </Typography>
        ),
      },
      {
      ...
      },
      {
        ...
        align: "center",
        headerAlign: "center",
        renderCell: ({ row }) => date(row.updatedAt),
      },
      {
        field: "actions",
        headerName: "Thao tác",
        ...
        renderCell: ({ row }) => (
          <Stack direction="row" justifyContent="flex-end" width="100%">
            <Button
              size="small"
              onClick={() => go({ to: `/image-contents/edit/${row.id}` })}
            >
              Sửa
            </Button>
            <ImageContentDeleteButton id={row.id} />
          </Stack>
        ),
      },
    ],
  ...
  const capacity = selectedType
    ? `${selectedType.itemCount} / ${selectedType.maxItems ?? "Không giới hạn"}`
    : "Chọn Type để xem giới hạn";
  return (
    <Box>
      <Stack
      ...
      >
        <Box>
          ...
          <Typography color="text.secondary">
            Quản lý ảnh và metadata theo Type.
          </Typography>
        </Box>
        <Stack direction="row" spacing={1}>
          <Button
            variant="outlined"
            startIcon={<SortIcon />}
            disabled={!selectedType || selectedType.itemCount < 2}
          ...
          <Button
            ...
            onClick={() =>
              go({
                to: `/image-contents/create${typeCode ? `?typeCode=${encodeURIComponent(typeCode)}` : ""}`,
              })
            }
          >
            Thêm nội dung
      ...
      {capacityReached && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Type đã đạt giới hạn nội dung. Hãy tăng giới hạn hoặc xóa một item
          trước khi thêm mới.
        </Alert>
      )}
      {tableQuery.isError && (
        ...
        headerButtons={() => (
          ...
          >
            ...
            <TextField
              ...
              sx={{ minWidth: 180 }}
            >
              <MenuItem value="">Tất cả Type</MenuItem>
      ...
      {typeCode && (
        <ImageContentReorderDialog
          open={reorderOpen}
          typeCode={typeCode}
          onClose={() => setReorderOpen(false)}
          onSaved={() => {
            void tableQuery.refetch();
            void types.query.refetch();
          }}
        />
      )}
    </Box>
