...
export const useSectionCollection = (sectionId: number, collection: string, itemType: PageSectionItemType) => {
  const { open } = useNotification();
  const list = useList<IPageSectionItem, HttpError>({ resource: "page-section-items", dataProviderName: "pageSections", pagination: { mode: "off" }, meta: { sectionId, collection } });
  ...
  const save = useCallback(async () => { await mutation.mutateAsync({ resource: "page-section-items", id: `${sectionId}:${collection}`, values: { sectionId, collection, itemType, itemIds: items.map((item) => item.itemId) } }); await list.query.refetch(); open?.({ type: "success", message: `Đã lưu collection ${collection}.` }); }, [collection, itemType, items, list.query, mutation, open, sectionId]);
