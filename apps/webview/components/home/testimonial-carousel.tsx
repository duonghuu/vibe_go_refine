  ...
  return (
    <div className="overflow-hidden">
      <div className="flex transition-transform duration-500" style={{ transform: `translateX(-${activeIndex * (100 / visibleCount)}%)` }}>
        {items.map((item, index) => <article key={`${item.name}-${item.quote}-${index}`} className="shrink-0 px-4 md:w-1/2" style={{ width: `${100 / visibleCount}%` }}><div className="relative py-10 pl-16 pr-5"><i className="ti-quote-left absolute left-5 top-5 text-4xl text-[#f75757]" /><p className="mb-7 text-xl italic leading-[1.9] text-[#242424]">{item.quote}</p><h3 className="mb-0 text-lg font-semibold text-[#242424]">{item.name}</h3><p className="text-[#808080]">{item.role}</p></div></article>)}
      </div>
